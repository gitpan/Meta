...
sub printd($$) {
	...
	if($self->size()>0) {
		...
		$writ->characters("No users are defined for this database");
	}
	$writ->endTag("para");
}
...
Meta::Db::Users - Object to store a hash of User objects for a database.
...
=head1 SYNOPSIS
	...
	my($enums)=Meta::Db::Users->new();
	my($user)=$users->get("mark");

=head1 DESCRIPTION

This is an object to store a list of User objects for a database.
...
This method receives a Users object and a collection of Sql statements.
...
users.
Since the Users object doesn't need any cleaning, this method does
nothing.
...
=head1 HISTORY
	...
	0.19 MV cook updates
	0.20 MV pictures in docbooks
	0.21 MV revision in files
	0.22 MV languages.pl test online
	0.23 MV history change
	0.24 MV db stuff
	0.25 MV more data sets
